...
export default function About() {
  ...
  return (
    <main className="min-h-screen w-full bg-deep-bg bg-space-pattern bg-cover bg-center bg-fixed flex items-center justify-center p-4 md:p-8 font-sans select-none overflow-hidden relative">

      {/* BACKGROUND EFFECTS */}
      {/* Top/Bottom Fade */}
      <div className="fixed top-0 left-0 w-full h-24 bg-gradient-to-b from-deep-bg via-deep-bg/90 to-transparent pointer-events-none z-10"></div>
      ...
      {/* MAIN DATA CARD */}
      <div className="relative w-full max-w-6xl h-[85vh] bg-[#0a1128]/80 backdrop-blur-xl border border-cyan-500/30 rounded-[30px] shadow-[0_0_50px_rgba(34,211,238,0.15)] overflow-hidden flex flex-col md:flex-row z-20 mt-12">
        
        {/* Decorative Header Line */}
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-cyan-500 to-transparent opacity-50"></div>

        {/* --- LEFT COLUMN: ID CARD / STATS --- */}
        <div className="w-full md:w-[35%] bg-black/20 border-r border-cyan-500/20 p-8 flex flex-col relative">
           ...
           <div className="space-y-1 mb-8 text-center md:text-left">
             ...
             <div className="flex items-center justify-center md:justify-start gap-2 text-cyan-400 font-mono text-sm">
           ...
           <div className="space-y-4 font-mono text-xs tracking-widest text-blue-200">
             <StatBar label="Frontend" percentage="95%" color="bg-cyan-400" />
             <StatBar label="Backend" percentage="85%" color="bg-purple-500" />
             <StatBar label="Creativity" percentage="100%" color="bg-pink-500" />
             <StatBar label="Caffeine" percentage="110%" color="bg-yellow-400" />
           </div>

           {/* Bottom Details */}
           <div className="mt-auto pt-8 flex gap-4 text-xs text-gray-500 font-mono uppercase">
             <div>ID: 8492-XJ</div>
        ...
        <div className="flex-1 p-8 md:p-12 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-4 mb-8">
             <FaTerminal className="text-3xl text-cyan-500" />
          ...
          {/* Bio Text */}
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-10 relative overflow-hidden">
          ...
          {/* Experience Grid */}
          <h3 className="text-xl text-cyan-400 font-bold mb-6 tracking-widest uppercase flex items-center gap-2">
...
const StatBar = ({ label, percentage, color }: { label: string, percentage: string, color: string }) => (
  <div className="w-full">
    ...
    <div className="w-full h-2 bg-black/40 rounded-full overflow-hidden border border-white/5">
      <div className={`h-full ${color} shadow-[0_0_10px_currentColor]`} style={{ width: percentage }}></div>
    </div>
  </div>
);
