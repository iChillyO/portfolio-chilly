...
const planStyles: { [key: string]: { icon: React.ReactNode; accent: string; btnClass: string; borderClass: string; popular?: boolean } } = {
  Scout: { icon: <FaRocket className="text-cerulean" />, accent: "text-cerulean", btnClass: "bg-white/[0.04] border border-white/[0.06] text-pearl", borderClass: "border-white/[0.06]" },
  Vanguard: { icon: <FaGem className="text-lilac" />, accent: "text-lilac", btnClass: "bg-cerulean text-deep-bg shadow-lg shadow-cerulean/20", borderClass: "border-cerulean/30", popular: true },
  Titan: { icon: <FaCrown className="text-gold" />, accent: "text-gold", btnClass: "bg-gold text-deep-bg shadow-lg shadow-gold/20", borderClass: "border-gold/20" },
};
...
export default function Pricing() {
  ...
  useEffect(() => {
    ...
    fetch('/api/profile', { signal: controller.signal })
      .then(res => res.json())
      .then(data => { if (data.success) setPlans(data.data.pricing); })
  ...
  return (
    <main className="min-h-screen bg-deep-bg font-sans select-none overflow-x-hidden relative text-pearl page-top pb-12">
      <div className="deco-blur absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-galaxy/30 rounded-full blur-[100px]" />
      <div className="max-w-5xl mx-auto section-padding relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {plans.map((plan, i) => {
            ...
            return (
              <div key={i} className={`relative glass-card p-6 flex flex-col ${style.borderClass}`}>
                ...
                <button onClick={() => setActivePlan(plan)} className={`w-full py-2.5 rounded-lg font-medium text-sm transition-colors ${style.btnClass}`}>Get Started</button>
              </div>
            );
          })}
        </div>
